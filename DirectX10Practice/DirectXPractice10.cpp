...
{
	_game.DeviceAcceptable(Adapter, Output,DeviceType,BackBufferFormat,bWindowed,pUserContext);
	return true;
}
...
{
	_game.ModifyDeviceSettings(pDeviceSettings, pUserContext);
	return true;
}
...
HRESULT CALLBACK OnD3D10CreateDevice(ID3D10Device* pd3dDevice, const DXGI_SURFACE_DESC* pBackBufferSurfaceDesc,
	void* pUserContext)
{
	_game.OnCreateDevice(pd3dDevice, pBackBufferSurfaceDesc, pUserContext);
	return S_OK;
}
...
{
	_game.OnResizedSwapChain(pd3dDevice, pSwapChain, pBackBufferSurfaceDesc, pUserContext);
	return S_OK;
}
...
{
	_game.OnFrameMove(fTime, fElapsedTime, pUserContext);
}
...
void CALLBACK OnD3D10FrameRender(ID3D10Device* pd3dDevice, double fTime, float fElapsedTime, void* pUserContext)
{
	...
	_game.OnFrameRender(pd3dDevice, fTime, fElapsedTime, pUserContext);
}
...
{
	_game.OnReleasingSwapChain(pUserContext);
}
...
{
	_game.OnDestroyDevice(pUserContext);
}
...
{
	_game.OnMsg(hWnd, uMsg, wParam, lParam, pbNoFurtherProcessing, pUserContext);
	return 0;
}
...
{
	_game.OnKeyboard(nChar, bKeyDown, bAltDown, pUserContext);
}
...
{
	_game.OnMouse(bLeftButtonDown, bRightButtonDown, bMiddleButtonDown, bSideButton1Down, bSideButton2Down, nMouseWheelDelta, xPos, yPos, pUserContext);
}
...
{
	_game.OnDeviceRemoved(pUserContext);
	return true;
}
